...
def filter_by(request):
    ...
    attributes = Property.objects.filter(name__in=values_s.keys(), description__in=l)
    product_list = Product.objects.filter(property__in=attributes, price__gte=minPrice, price__lte=maxPrice).distinct()
